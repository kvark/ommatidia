...
pub(super) fn frame(
    ...
    let solution = oracle::reconstruct(&input, &target.lobes)?;
    let n = frame.surfaces.len();
    let mut learned = vec![0.0; 6 * n];
    native.session.read_output_by_index(0, &mut learned);
    let value = |k: usize, j: usize| {
        if k == SCALES {
            input.history[j]
        } else {
    ...
    for l in 0..2 {
        for i in 0..n {
            let sum = (0..CANDIDATES)
                .map(|k| input.prior[k * 2 * n + l * n + i])
                .sum::<f32>()
                .max(1e-12);
            for c in 0..3 {
                let j = (3 * l + c) * n + i;
                fixed[j] = (0..CANDIDATES)
                    .map(|k| input.prior[k * 2 * n + l * n + i] * value(k, j) / sum)
                    .sum();
            }
        }
    }
    let mse = |image: &[f32]| -> [f64; 2] {
        std::array::from_fn(|l| {
            (0..3 * n)
    ...
    let signed = |image: &[f32]| -> [f64; 6] {
        std::array::from_fn(|c| {
            (0..n)
                .map(|i| image[c * n + i] as f64 - target.lobes[c * n + i] as f64)
                .sum::<f64>()
                / n as f64
        })
    };
    let shares = |weights: &[f32]| -> Vec<[f64; CANDIDATES]> {
        (0..2)
            .map(|l| {
                std::array::from_fn(|k| {
                    (0..n)
                        .map(|i| weights[k * 2 * n + l * n + i] as f64)
                        .sum::<f64>()
                        / n as f64
                })
            })
    ...
    for i in 0..n {
        for c in 0..3 {
            let d = solution.lobes[config.index(frame.low, c, i % width, i / width)];
            let s = solution.lobes[config.index(frame.low, 3 + c, i % width, i / width)];
            rgb[3 * i + c] =
                d * frame.surfaces[i].albedo_roughness[c] + s + frame.surfaces[i].emission[c];
        }
    }
    save_png(&out.join(format!("{prefix}-oracle.png")), &rgb, extent)?;
    Ok(
        serde_json::json!({"fixed_state":"actual learned-history GPU inputs; never oracle feedback",
