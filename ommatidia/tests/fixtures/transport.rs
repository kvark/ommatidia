...
fn fixture(config: Config, step: usize, seed: u64) -> (Frame, Target) {
    ...
    let truth = |x: f32, y: f32| {
        let foreground = x >= 4.0 + step as f32
            && x < 8.0 + step as f32
            && y > 3.0
            && y < 12.0;
        let mut s = Surface {
            normal_depth: [0.0, 0.0, 1.0, if foreground { 0.5 } else { 1.0 }],
            albedo_roughness: [if foreground { 0.8 } else { 0.4 }, 0.6, 0.3, 0.6],
            ..Surface::default()
        };
    ...
    for y in 0..width {
        for x in 0..width {
            let (s, d, sp) = truth(x as f32 + 0.5, y as f32 + 0.5);
            frame.surfaces.push(s);
            for c in 0..3 {
                target.lobes[config.index(low, c, x, y)] = d[c];
                target.lobes[config.index(low, 3 + c, x, y)] = sp[c];
                target.rgb[(y * width + x) * 3 + c] =
                    d[c] * s.albedo_roughness[c] + sp[c] + s.emission[c];
            }
        }
    }
    for y in 0..low[1] {
        for x in 0..low[0] {
            let (s, d, sp) = truth(
                (x as f32 + 0.5 + jitter[0]) * config.scale as f32,
                (y as f32 + 0.5 + jitter[1]) * config.scale as f32,
            );
            let noise = -rng.uniform().max(1e-6).ln();
            let noise2 = -rng.uniform().max(1e-6).ln();
            frame.rays.push(Ray {
