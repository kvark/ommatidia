...
fn extreme_logits_preserve_candidate_mass_and_cpu_reconstruction() {
    ...
    for offset in [-1000.0f32, -30.0, -20.0, -18.5, -17.0, -16.0, 0.0, 30.0, 1000.0] {
        ...
        let reference: Vec<_> = logits.iter().map(|&x| {
            let x = x as f64;
            (x.max(0.0) + (-x.abs()).exp().ln_1p()) as f32
        }).collect();
        for (&actual, &truth) in multipliers.iter().zip(&reference) {
            if truth >= transport::MIN_MULTIPLIER {
                assert!((actual - truth).abs() <= 1e-5 * truth, "softplus negative-tail mismatch at offset {offset}: {actual} != {truth}");
            }
        }
