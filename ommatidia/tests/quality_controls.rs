...
use ommatidia::{field, transport::{cpu, graph}};
use std::sync::Arc;
include!("fixtures/transport.rs");

#[test]
fn stratification_preserves_intervals_and_target_isolation() {
    let bounds = field::Bounds { center: [0.0; 3], radius: 1.0 };
    let rays = [
        field::Ray { origin: [0.0, 0.0, -3.0], direction: [0.0, 0.0, 1.0] },
        field::Ray { origin: [3.0, 3.0, -3.0], direction: [0.0, 0.0, 1.0] },
    ];
    ...
    assert_eq!(dt, a.1);
    assert_eq!(a.1, b.1);
    for (x, y) in a.0.iter().zip(&b.0) { assert_eq!(x.position, y.position); }
    ...
    let t = field::surface::Targets::new(bounds, &rays, 8,
        &[Some((Some(2.7), 20.0)), None], 1.0).unwrap();
...
fn loss_controls_are_validated_and_training_only() {
    ...
    assert!(graph::LossWeights { compressed: f32::NAN, ..good }.validate().is_err());
    assert!(graph::LossWeights { temporal: -0.1, ..good }.validate().is_err());
    ...
    assert_eq!(m.params.iter().map(|p|(&p.name,p.len)).collect::<Vec<_>>(),
        training.params.iter().map(|p|(&p.name,p.len)).collect::<Vec<_>>());
}
...
fn feed(session: &mut meganeura::Session) {
    ...
    for slot in 0..2 {
        ...
        session.set_input(&format!("f{slot}.loss_scale"), &vec![1.0; target.lobes.len()]);
...
fn objective_weights_are_linear_and_physical_only_training_learns() {
    ...
    for (i, term) in terms.iter_mut().enumerate() {
        let mut weights = [0.0; 5]; weights[i] = 1.0;
        forward.set_input("loss.weights", &weights);
        forward.step(); forward.wait(); *term = forward.read_loss();
        assert!(term.is_finite() && *term >= 0.0);
    }
    graph::LossWeights::default().feed(&mut forward);
    forward.step(); forward.wait();
    let expected = terms.into_iter().zip([1.0,0.1,0.05,0.01,0.01]).map(|(t,w)|t*w).sum::<f32>();
    ...
    model.initialize(&mut train, 7); feed(&mut train);
    graph::LossWeights { compressed: 0.0, confidence: 0.0, temporal: 0.0,
        ..Default::default() }.feed(&mut train);
    train.set_adam(1e-3,0.9,0.999,1e-8);
    let mut first = 0.0; let mut last = 0.0;
    for i in 0..16 {
        train.step(); train.wait(); last = train.read_loss();
        assert!(last.is_finite()); if i==0 { first=last; }
    }
    assert!(last < first, "physical-only objective did not learn: {first} -> {last}");
