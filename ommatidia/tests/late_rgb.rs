...
fn common_parameters_keep_order_and_views_share_weights() {
    let c = config();
    let late = graph::build_points(&c, 2).unwrap();
    let mut old = c.clone();
    old.view_fusion = ViewFusion::Moments;
    let old = graph::build_points(&old, 2).unwrap();
    for (a, b) in old.params.iter().zip(&late.params) {
        assert_eq!(a.name, b.name);
        assert_eq!(a.len, b.len);
    }
    assert!(late.params.len() > old.params.len());
    let mut three = c;
    three.views = 3;
    let other = graph::build_points(&three, 2).unwrap();
    assert_eq!(late.params.len(), other.params.len());
    assert!(late.graph.nodes().iter().all(|n| match &n.op {
        meganeura::graph::Op::Input { name } => !name.starts_with("target."),
        _ => true,
    }));
}
#[test]
...
fn linear_sources_direction_isolation_and_missing_coverage() {
    ...
    let m = graph::build_points(&c, 3).unwrap();
    let mut s =
    ...
    Prepared::new(&obs, &c, &queries).unwrap().feed(&mut s);
    s.step();
    s.wait();
    let sigma = s.read_output(3);
    assert!((sigma[0] - sigma[1]).abs() < 1e-6);
    let mut rgb = vec![0.0; 9];
    s.read_output_by_index(1, &mut rgb);
    for v in &rgb[..6] {
        assert!((*v - 4.0).abs() < 1e-4, "linear source blend {v}");
    }
    assert!(rgb.iter().all(|v| v.is_finite() && *v >= 0.0));
    let mut swapped = obs.clone();
    swapped.views.reverse();
    Prepared::new(&swapped, &c, &queries).unwrap().feed(&mut s);
    s.step();
    s.wait();
    let mut reordered = vec![0.0; 9];
    s.read_output_by_index(1, &mut reordered);
    assert!(
        rgb.iter()
            .zip(&reordered)
    ...
    let mut changed = obs;
    for v in &mut changed.views {
        v.rgb.fill(1000.0);
    }
    Prepared::new(&changed, &c, &queries).unwrap().feed(&mut s);
    s.step();
    s.wait();
    s.read_output_by_index(1, &mut reordered);
    assert!(
...
fn late_rgb_training_updates_and_reloads_view_parameters() {
    ...
    let mut s = ommatidia::gpu::training_session(&m.graph, context.clone());
    m.initialize(&mut s, 7);
    let rays = [
    ...
    let mut weights = vec![0.0; c.hidden as usize];
    s.read_param("field.source.score.out.weight", &mut weights);
    assert!(weights.iter().any(|v| v.abs() > 1e-6));
    let path = std::env::temp_dir().join(format!("late-rgb-{}.safetensors", std::process::id()));
    s.save_checkpoint(&path).unwrap();
    let inference = graph::build_render(&c, shape, false).unwrap();
    let mut restored = ommatidia::gpu::inference_session(&inference.graph, Arc::clone(&context));
    restored.load_checkpoint(&path).unwrap();
    Prepared::new(&obs, &c, &q).unwrap().feed(&mut restored);
    restored.set_input("ray.deltas", &dt);
    restored.step();
    restored.wait();
    assert!(restored.read_output(6).iter().all(|v| v.is_finite()));
    std::fs::remove_file(path).unwrap();
    println!("late RGB learning loss {first} -> {last}");
}
