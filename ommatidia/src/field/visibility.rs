...
pub fn survival(bounds: Bounds, origin: [f32; 3], position: [f32; 3]) -> [f32; BINS + 1] {
    ...
    if !distance.is_finite() || distance <= 1e-6 { return [0.0; BINS + 1]; }
    let ray = Ray { origin, direction: unit(delta) };
    let (near, far) = data::ray_interval(bounds, ray);
    if far <= near { return [0.0; BINS + 1]; }
    let u = (distance - near) / (far - near) * BINS as f32 - 0.5;
    std::array::from_fn(|bin| if bin == BINS { 1.0 } else { (bin as f32 + 1.0 - u).clamp(0.0, 1.0) })
}
...
pub struct Targets { pub mass: Vec<Vec<f32>>, pub valid: usize }
impl Targets {
    pub fn new(obs: &Observations, config: &Config, captures: &[Option<surface::Capture>], weight: f32) -> Result<Self, String> {
        obs.validate(config)?;
        ...
        for (view, capture) in obs.views.iter().zip(captures) {
            ...
            for p in 0..n {
                let ray = view.camera.ray([(p % w as usize) as f32, (p / w as usize) as f32], config.extent);
                if let Some(bin) = surface::class(obs.bounds, ray, capture.distance[p], capture.ray_limit, BINS) {
        ...
        for row in &mut mass { row.iter_mut().for_each(|v| *v *= scale); }
        Ok(Self { mass, valid })
    }
