...
fn solve(mut a: [[f64; 4]; 3], n: usize) -> Option<[f64; 3]> {
    let scale = a
        .iter()
        .take(n)
        .flat_map(|r| r.iter().take(n))
        .map(|v| v.abs())
        .fold(0.0f64, f64::max);
    for c in 0..n {
        let pivot = (c..n).max_by(|i, j| a[*i][c].abs().total_cmp(&a[*j][c].abs()))?;
        if a[pivot][c].abs() <= 1e-12 * scale {
            return None;
        }
        a.swap(c, pivot);
        let d = a[c][c];
        for value in &mut a[c][c..=n] {
            *value /= d;
        }
        let pivot = a[c];
        for (i, row) in a.iter_mut().take(n).enumerate() {
            if i == c {
                continue;
            }
            let d = row[c];
            for (value, p) in row[c..=n].iter_mut().zip(&pivot[c..=n]) {
                *value -= d * p;
            }
        }
    }
...
pub fn project(candidates: &[[f32; 3]], target: [f32; 3]) -> Result<Projection, String> {
    ...
    let points: Vec<_> = candidates.iter().map(|p| p.map(f64::from)).collect();
    let target = target.map(f64::from);
    let mut best = points[0];
    let mut error = dot(sub(best, target), sub(best, target));
    ...
    for mask in 1usize..(1 << points.len()) {
        ...
        let ids: Vec<_> = (0..points.len()).filter(|i| mask & (1 << i) != 0).collect();
        let origin = points[ids[0]];
        let n = count - 1;
        let basis: Vec<_> = ids[1..].iter().map(|i| sub(points[*i], origin)).collect();
        let mut a = [[0.0; 4]; 3];
        for i in 0..n {
            for j in 0..n {
                a[i][j] = dot(basis[i], basis[j]);
            }
            a[i][n] = dot(basis[i], sub(target, origin));
        }
        let Some(x) = solve(a, n) else {
            continue;
        };
        let mut w = vec![1.0 - x[..n].iter().sum::<f64>()];
        w.extend_from_slice(&x[..n]);
        if w.iter().any(|v| *v < -1e-9 || !v.is_finite()) {
            continue;
        }
        ...
        let point =
            std::array::from_fn(|c| ids.iter().zip(&w).map(|(i, w)| w * points[*i][c]).sum());
        let e = dot(sub(point, target), sub(point, target));
        if e < error {
            best = point;
            error = e;
            weights.fill(0.0);
            for (i, w) in ids.iter().zip(w) {
                weights[*i] = w as f32;
            }
        }
    }
    let gradient = sub(best, target);
    let gap = points
        .iter()
        .map(|p| dot(gradient, sub(best, *p)))
        .fold(0.0f64, f64::max);
    Ok(Projection {
        point: best.map(|v| v as f32),
        weights,
        squared_error: error,
        dual_gap: gap,
    })
...
pub fn reconstruct(input: &Candidates, target: &[f32]) -> Result<Reconstruction, String> {
    ...
    let mut result = Reconstruction {
        lobes: vec![0.0; 6 * n],
        weights: vec![0.0; CANDIDATES * 2 * n],
        lobe_mse: [0.0; 2],
        max_dual_gap: 0.0,
    };
    for l in 0..2 {
        for i in 0..n {
            let ids: Vec<_> = (0..CANDIDATES)
                .filter(|k| input.prior[k * 2 * n + l * n + i] > 0.0)
                .collect();
                ...
                .map(|k| {
                    std::array::from_fn(|c| {
                        ...
                        if *k == SCALES {
                            ...
                            input.spatial[k * 6 * n + j]
                        }
                    })
                })
                .collect();
            let truth = std::array::from_fn(|c| target[(3 * l + c) * n + i]);
            let p = project(&points, truth)?;
            for c in 0..3 {
                result.lobes[(3 * l + c) * n + i] = p.point[c];
            }
            for (k, w) in ids.into_iter().zip(p.weights) {
                result.weights[k * 2 * n + l * n + i] = w;
            }
            result.lobe_mse[l] += p.squared_error / (3 * n) as f64;
            result.max_dual_gap = result.max_dual_gap.max(p.dual_gap);
        }
    }
    Ok(result)
}
